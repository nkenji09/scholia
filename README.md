...
`scholia rules` and `scholia spec` show only what is still in force. A withdrawn decision is not silently dropped: its existence and where it was replaced stay in the output, and `--all` brings the full text back.
By default `scholia rules` prints the **full text** of only those decisions that target the record you asked about. Rules that reach it through a tag — decisions on an ancestor tag, or on the requirement a transition belongs to — come back as **existence, the tag they arrive through, and how to read them**: the prose was written in one place, so it is read in one place. When a record has no decisions of its own, the output does not say "none" — it reports how many rules govern it through tags and how to fetch them. Use `--all` to expand everything. `scholia search` never folds anything — it marks withdrawn hits instead, so you can still find them. Every JSON payload carries an `effect` field (`in-force` / `replaced`) so a machine reader does not have to reconstruct the reverse links itself. `scholia decision list --unlinked` finds decisions with no commit landed yet — useful when auditing follow-through.
