...
const ja = {
  ...
  nav: {
    ...
    vocab: '語彙',
    config: '設定',
    // 評価コックピット（diff-viz・§2）。2026-07-11 に一度撤去されたが
    // 2026-07-12 に別枠（比較/評価ビュー）として再導入（G-5 承認済み）。
    // サーバモード限定 — Header.tsx がここを isStaticMode の時ナビから外す。
    compare: '評価',
  },
  header: {
    fontDec: '文字を小さく',
  ...
  // 評価コックピット（比較/diff-viz view・§2）。read-only。base/head の
  // gitref を選び `GET /api/diff` を叩いて意味 diff を見せる。サーバモード
  // 限定（static export では非表示・CompareView.tsx 参照）。
  compare: {
    heading: '評価（比較）',
    intro: 'base と head の間で、語彙・タグ・遷移・意思決定が何を変えるかを見る。',
    staticUnavailableTitle: 'サーバモード限定',
    staticUnavailableBody: '評価ビューは pmem view（サーバモード）でのみ利用できます。静的書き出し（export --html）には比較対象の ref データが含まれないためです。',
    baseLabel: 'base（gitref）',
    headLabel: 'head（空欄 = 作業ツリー）',
    headPlaceholder: '空欄で pending（作業ツリー）',
    run: '比較する',
    running: '比較中…',
    pendingNote: (base: string) => `作業ツリー vs ${base}（pending・未着地）`,
    landedNote: (base: string, head: string) => `${head} vs ${base}（landed・commit 対 commit）`,
    baselineMissingNote: 'ベースラインが見つからないため、現在の全レコードを新規（added）として表示しています（初回想定のフォールバック）。',
    empty: '差分はありません。',
    decisionViolationHeading: '⚠ decision の削除／改変を検出しました（append-only 違反）',
    decisionViolationBody: 'decision は追記専用（判断は不変・§3.5）です。この変更は取り込み前に要再考です。',
    vocabHeading: '語彙 ±',
    tagsHeading: 'タグ ±',
    transitionsHeading: '遷移 ±',
    decisionsHeading: '意思決定 ±',
    untaggedGroup: '（タグなし）',
    added: '追加',
    removed: '削除',
    changed: '変更',
    thenReordered: 'then 順序変更',
    thenChangedLabel: 'then',
    givenChangedLabel: 'given',
    tagsChangedLabel: 'tags',
    testsChangedLabel: 'tests',
    actionChangedLabel: (before: string, after: string) => `action: ${before} → ${after}`,
    decisionRemovedLabel: '削除（append-only 違反）',
    decisionChangedLabel: '改変（append-only 違反）',
  },
...
const en: Strings = {
  ...
  compare: {
    heading: 'Review (compare)',
    intro: 'See what vocab, tags, transitions, and decisions change between base and head.',
    staticUnavailableTitle: 'Server mode only',
    staticUnavailableBody: 'The review view is only available in server mode (pmem view) — a static export bakes in one snapshot and has no other ref to compare against.',
    baseLabel: 'base (gitref)',
    headLabel: 'head (blank = working tree)',
    headPlaceholder: 'Blank = pending (working tree)',
    run: 'Compare',
    running: 'Comparing…',
    pendingNote: (base) => `Working tree vs ${base} (pending — not yet landed)`,
    landedNote: (base, head) => `${head} vs ${base} (landed — commit vs commit)`,
    baselineMissingNote: 'No baseline was found, so every current record is shown as newly added (first-run fallback).',
    empty: 'No differences.',
    decisionViolationHeading: '⚠ Detected decision removal/modification (append-only violation)',
    decisionViolationBody: 'Decisions are append-only (judgments are frozen — §3.5). This change needs reconsideration before landing.',
    vocabHeading: 'Vocab ±',
    tagsHeading: 'Tags ±',
    ...
    removed: 'Removed',
    changed: 'Changed',
    thenReordered: 'then reordered',
    thenChangedLabel: 'then',
    givenChangedLabel: 'given',
    tagsChangedLabel: 'tags',
    testsChangedLabel: 'tests',
    actionChangedLabel: (before, after) => `action: ${before} → ${after}`,
    decisionRemovedLabel: 'Removed (append-only violation)',
    decisionChangedLabel: 'Modified (append-only violation)',
  },
  lookups: {
  ...
  api: {
    ...
    diff: 'the review (compare) view diff',
  },
};
