...
export const TAGS: Tag[] = [
  ...
  { id: 'axis.mode', name: 'モード軸', kind: 'axis', parentIds: ['comp.cli'] },
  // ⚠️ **「開閉できない束ねる段」**。これが無いと、行の指し先を決める材料を
  // 「役割を持つ子の数」から「全部の子の数」へ戻す変異が素通りする——
  // その変異は**三角も出ず・リンクでもない行**（＝押しても何も起きない行）を作るが、
  // それが起きるのは「子は居るが、役割を持つ子は居ない構造ノード」だけである。
  // 実データではこの形が4件あった（要件の子だけを持つ要件タグ）。
