...
export const DEC = {
  ...
  D10: '01HARNESSD100000000000000A',
} as const;
...
export const TAGS: Tag[] = [
  ...
  // ⚠️ **3つ目の親に、居場所にならない要件タグを混ぜてある。** これが無いと、開示から
  // 「居場所の役割だけを出す」判定を外す変異が**素通りする**（実見）——他の構成要素は
  // 役割を持つ親しか持たないので、判定を外しても開示の中身が変わらないため。
  { id: 'part.shared.pair', name: '共有: 位置の復元', kind: 'part', parentIds: ['part.list', 'part.detail', 'req.viewer.nav'] },
  // ⚠️ **記録の1つ目の親の道が行き止まりの多親。** 上にコンポーネントが1つも居ない構成要素
  // （親が要件タグだけ＝どのシートにも出ない）を1つ目の親に置いてある。これが無いと、
  // **「選んだ道にコンポーネントが無ければそこで諦める」形が素通りする**——その形では
  // 共有 URL が転送されず、**欄が別のシートに実在するのに既定のシートを黙って出す**
  // （`01KYPFJV04R347HWHQKQ2TW275` が「一番悪い」と名指しした状態）。
  // ⚠️ この2件はツリーにも欄にも出ない親を持つので、**ツリーの行の集合は変わらない。**
  { id: 'part.orphan', name: '行き止まりの構成要素', kind: 'part', parentIds: ['req.tools'] },
  // ⚠️ **行き先の候補を2つ持たせてある**（`part.detail`→ビューア画面 と `part.export.job`→書き出し）。
  // 1つしか無いと、**他の道を探す順を逆にする変異が描画側で素通りする**（実見）——どちらから
  // 探しても同じ答えになるため。2つあれば「記録の親の順が答えを決める」ことが値で見える。
  { id: 'part.shared.deadfirst', name: '共有: 1つ目が行き止まり', kind: 'part', parentIds: ['part.orphan', 'part.detail', 'part.export.job'] },
