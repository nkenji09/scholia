  ...
  return (
    <article ref={cardRef} data-card-id={tag.id} class="card" title={tag.id}>
      ...
      {/* 継承した規則の開示（01KYHW4NBNVN9BFXYZMBX8MPF8 条項3）: 「この記録を
          ...
          件数と継承元と導線だけを出す。あわせて「その全体をどこで読めるか」
          （追補 01KYJV3FYMDFRWQ939NBV2BPAC 条項3）もここが出す。 */}
      <InheritedRules record={{ kind: 'tag', id: tag.id }} />

      {/* 「このタグと配下に付いた意思決定」の一覧への入口。**支配する規則（自身＋
          祖先）を指す入口ではない**——一覧は配下方向にしか絞れない（追補 条項2）。
          それ自体は使える眺めなので、タグは一覧の絞り込み単位そのものであることを
          活かして常に出す（継承0件でも）。 */}
