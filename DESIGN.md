...
- `Decision { id, target: {type: "transition"|"tag"|"vocab", id}, why, changed?, ref?, commits?[], acknowledges?[], supersedes?[{id, mode}], at }` — **append-only**（**取り込み先に載ったら**消さない・直さない。訂正は新しい 1 件を足す）。
  🔴 **凍るのは「作った瞬間」ではなく「取り込み先（base）に載った時点」である**（01M1N5YPTRSYAJWH3W7GK4FP64）。
  歯止め（`diff --check <base>`）は最初からそう動いており、CI も `origin/<base_ref>` と比べる——ブランチの中だけに在る decision は
  「増えた 1 件」でしかない。載る前は `scholia decision edit <id> [--why|--changed|--ref]` と `scholia decision rm <id>` で直せる
  （どちらも **base に既に在れば拒否**し、**base を決められなければ拒否**する＝fail closed。`target`／`at` は直せない）。
  ⚠️ **push 済みの共有ブランチでの書き換えは止められない**——見ているのは「base に在るか」だけである。`acknowledges` は #45 D6・`supersedes` は #45 D7 の追加フィールド（いずれも additive/omitempty・既存レコードは無改修）。
...
- **`commits[]`（git hash の集合・任意）も実装来歴。ただし非推奨**（01M1K4WPN3HXNVE0CR0T6NQK33）。
  ...
  - **結ぶ commit は保存前に照合される**: 形（16 進 7〜64 文字）はどこでも、実在（`git rev-parse` が commit として解決すること）は
    ...
    「照合していない」と出す（**CLI のテキスト面・CLI の `--json` 封筒・viewer の `POST /api/decision` 応答**のいずれにも出る）。
  - ⚠️ **16 進の名前を持つブランチ/タグは通さない。** `git rev-parse` は ref を先に解決するので、そのままだと
    **打った人が指していない commit が保存される**（実測）。解決した完全 hash が入力の接頭辞になっていることまで確かめる。
...
scholia tag list [--kind <k>] [--tree] [--json] [--all]      # --tree でネスト表示。--json の既定は description を畳む（--tree と併せても同じ。全文は --all か show tag・01KZ5ACN6P279S96D5M3AHY9HZ ＋ 追補 01KZ7YBTQ8STQMBHGFXG3CES44）
...
scholia decision edit <decisionId> [--why|--changed|--ref] [--base <ref>]  # 取り込み先に載っていない decision の判断を直す。base に在れば拒否・base を決められなければ拒否（01M1N5YPTRSYAJWH3W7GK4FP64）
scholia decision rm <decisionId> [--base <ref>]                           # 同上の条件で decision を消す
...
- **貫く原理**: ログは `.scholia` の中身を写さない。「どの記録を、どう引いて、どれだけ渡したか」は持ち、
  ...
  計測ログは真実の源ではなく**捨ててよい副産物**である——だから `.scholia/` 配下ではなくリポジトリの外に置く。
- **置き場所の規則は 1 つ**: `<state>/scholia/usage.jsonl`（JSON Lines・追記のみ）。
  `<state>` は `$XDG_STATE_HOME`（**絶対パスのときだけ**）、無ければ `$HOME/.local/state`。
  - **なぜ state か**: 数か月かけて貯めて判断材料にする前提のデータなので、**OS が掃除しうるキャッシュ配下には置かない**。
    捨ててよい副産物ではあるが、それは「消えても `.scholia/` が損なわれない」という意味であって、
    「積極的に消されてよい」ではない。
  - **なぜ OS で分岐しないか**: `os.UserConfigDir` / `os.UserCacheDir` は darwin と linux で別の場所を返すので、
    置き場所を検査するガードも OS で分岐することになる。規則を 1 つにするほうがガードを 1 本にできる。
  - ⚠️ **相対パスの `$XDG_STATE_HOME` は無効として無視する**（XDG の規定どおり）。作法の問題ではない
    ——そのまま使うと基点が cwd になり、**ログがプロジェクトの中に落ちる**。
  - ⚠️ **`.scholia` という名前のディレクトリを途中に作らない。** `store.Discover` は名前が `.scholia` の
    ディレクトリを**中身を見ずに**ストアとして拾うので（`internal/store/store.go`）、たとえばホーム直下に
    作ると「`$HOME` 配下の非プロジェクトから走らせた scholia がそれをストアとして開こうとする」状態を
    **計測をオフに戻した後も**残す。
  - **基点が決まらないとき**（`$XDG_STATE_HOME` が無効で `$HOME` も取れない）は**推測しない**。
    `DefaultPath` がエラーを返し、`Record` がそれを飲み込んで何も記録しない（記録の失敗は本業を落とさない）。
    ⚠️ **既定 off の不変には触れない**——`DefaultPath` を呼ぶのは `Record` だけで、`Record` はオフを先に弾く。
