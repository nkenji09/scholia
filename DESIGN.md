...
CLI の how とは別に、**何を登録するか**で価値が決まる。粒度・同一性・命名・desc/decision の役割分担・派生ビュー・記録を書く前のチェックは authoring の正典 `agents/skills/_scholia-shared/references/modeling-principles.md`（各スキルから参照）に従う。以下はその上での網羅の勘所:
