...
- **意思決定は取り込み先に載ったら append-only**：**取り込み先（main 等）に載った** decision は消さず直さず、訂正は 1 件足す（載る前ならブランチの中で `scholia decision edit` / `rm` で直せる・01M1N5YPTRSYAJWH3W7GK4FP64）。訂正は元の decision を「置き換える／部分改訂する／例外とする」（`supersedes`）と機械可読に宣言できるので、「今どれが正か」も辿れる。凍結された判断が、変更を評価する基準になる。
