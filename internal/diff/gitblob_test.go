...
// （閾値が標本の上端までなら）。
//
// ⚠️ 閾値の境界は1件単位で検算してある。`len(paths) > K` の形のゲートは
// **K ≤ 600 で落ち、K ≥ 601 で生き残る**（実測: 598・599・600 は red、601 は緑）。
// 600 件の fixture が返す paths は config.json を含めて 601 本なので、境界は
// レコード件数ではなく paths の本数の側に立つ。
//
