...
//     🔴 **新しい面だけでなく、既存の面に足した「ある bool フラグが立っているときだけ
//     通さない」枝でも落ちる。** 覆いは 2 つの経路から来る:
//     (a) 突き合わせを `executeForTest`（cli_test.go）に置いてあるので、
//     **テスト一式がこの package で叩く起動は全部そのまま対象になる**。
//     (b) さらに TestEveryJSONFaceGoesThroughTheSingleExit が、面ごとに
//     **宣言された bool フラグの全部分集合**を回す（実測 105 通り）。
//     ⚠️ (a) だけだと覆いが「他のテストがたまたま叩いた引き方」に依存し、
//     そのテストが消えると黙って覆いも消える。(b) は覆いを**宣言から導く**。
...
//
// **落ちない（射程の外・正直に名乗る）:**
//   - 🔴 **bool フラグ以外で表現される枝**——位置引数や文字列フラグの値で分岐する枝は、
//     どのテストもその引き方を叩かない限り通らない。
//     ⚠️ **実際に変異を書いて緑を実見してある**: `config get <key> --json`
//     （キーを指定した枝）で共有の口を通さないようにしても、**テスト一式は緑のまま**。
//     この面は 4 つの口を持ち、覆えているのはキー省略の 2 つ（素と `--local`）だけである。
//     ⚠️ **これは直しても残る穴である**——`executeForTest` へ移す前は
//     「面ごとに 1 つの引き方」しか走らせておらず、`rules --all` の枝に置いた
//     素通りが緑で通った（実見）。bool フラグの全部分集合まで広げても、
//     **位置引数の分岐までは宣言から導けない。** 塞ぐならその枝を叩くテストが要る。
