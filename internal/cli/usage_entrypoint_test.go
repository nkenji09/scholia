...
//     (b) **表の行を残したまま**、入口の検査を 1 本でも消す・スキップする。
//     ⚠️ 初版はパッケージ全体で「1 回以上走ったか」しか見ていなかったので、
//     **3 本のうち 1 本を消しても残り 2 本が数を埋めて沈黙した**
//     （消えたのは条項 10 を本番の入口で見る唯一の検査だった）。いまは下の表と名前で突き合わせる。
//     ⚠️ **検査と表の行を対で消せば沈黙する**——下の「落ちない」に書いた残余である。
//     (c) **表の回数を据え置いたまま**、1 本の中の再実行を 1 回でも減らす。
//     ⚠️ **検査が緑のままでも落ちる**——`StaysOffByDefault` の対照（段を立てた 1 回）を
//     まるごと消すと、その検査自体は緑で通るが回数が 1 になって落ちる。
//     ⚠️ こちらも**回数の数字と対で消せば沈黙する**（同上）。
...
//   - 🔴 ⚠️ **要求（表）と申告（検査）を対で消すこと。1 組でも沈黙する。**
//     初版はここを「表を空にし、かつ検査を**全部**消したときだけ」と書いていたが、**実際より狭い**
//     ——レビュアの実測で、**検査 1 本と表の 1 行**を対で消しても、
//     **回数の数字 1 文字と対応する再実行 1 か所**を対で消しても、どちらも緑だった。
//     突き合わせているのは表と申告の 2 つだけなので、**両方を同じ向きに動かせば必ず一致する。**
//     ⚠️ **片側だけを崩せば落ちる**——表だけ動かせば (d)、検査だけ消せば (b)、回数だけ動かせば (c)。
//     **これは表を持つ歯止めに共通の原理的な残余で、この形では閉じられない。**
//     閉じるには「呼び出し箇所が表と同数か」をソースで数えることになるが、それは
//     **同じ意味を別の綴りで書かれれば捕まらない**照合であり、CLAUDE.md 2 が禁じている。
//     **だから閉じずに、ここに名乗る。**
