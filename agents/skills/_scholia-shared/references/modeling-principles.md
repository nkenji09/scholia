...
この文書を**別 repo のスキルから読ませるときは、パスを書かず `scholia skills show modeling-principles` を
手順番号に置く**。相対パスが解決するのは同一プラグイン／同一 install 先の内側だけで、プラグイン実体のパスは
バージョンを含むため直書きは更新で腐る。
...
3. **構造制約 = 性質型要件**（action→effect でない・「〜しない」系・単一バイナリ等の非機能要件）。これは `scholia tag edit <id> --fulfillment property` で性質型と宣言し、**かつ** `scholia decide --on tag:<id> --acknowledges requirement-gap --why "<見出し＋本文>"` を足す。**property 宣言だけでは畳まない**（宣言のみ・decision 無しは warn のまま＝怠慢な宣言を許さない）。
   ⚠️ `--why` は **1 行目が `#` ＋ 1〜80 字の見出し・2 行目以降に本文**でないと保存時に拒否される（`decision-heading`。`why` は**取り込み先に載ったら**直せないため。載る前は `scholia decision edit` で直せる）。
