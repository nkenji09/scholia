...
2. コメントが指す対象（tag/transition の id）と、この変更が **Case 1（Tag の要件変更）**・
   **Case 2（Transition の修正）**・**Case 3（設計/横断 decision）** のどれかを確認する。両方（複数）に
   跨る場合は 1 task に集約し、該当する手順を順に踏む。
3. この decision が **prospective（変更を先導する）** か **retrospective（既に landing 済みの実装を
   後から記録する）** かを確認する。
...
- コメントの対象を Case 1／Case 2／Case 3 に分類し、跨る場合は 1 task に集約している。
