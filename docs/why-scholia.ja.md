...
- **過去の why を機械可読に引ける**：`scholia rules` が守るべき規則を、`scholia decision list` が過去の判断を返す。作業の前にこれを読み込めば、過去の意図に沿って動ける。散文を要約させる必要がない。既定で本文が出るのは**その記録自身への決定だけ**で、タグ経由で届く規則は存在と引き方が出る（全部読むなら `--all`）。
...
`scholia rules --tag <id> --all` で対象を支配している規則を引き、`scholia show vocab` で語彙の使用箇所を逆引きし、さらに素の `grep` でも `.scholia/` を横断できる。
